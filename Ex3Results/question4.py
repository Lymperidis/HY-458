...
class CommunicationChannel:
    ...
    def get_public_key(self) -> RSAPublicKey:
        ...
        attacker_private_key = generate_private_key(0x10001, 4096)
        attacker_public_key = attacker_private_key.public_key()

        # Save attacker's private key for later use
        self.attacker_private_key = attacker_private_key

        # Return attacker's public key instead of the server's
        return attacker_public_key
        #pass
    ...
    def transmit(self, message: bytes):
        ...
        plaintext = self.attacker_private_key.decrypt(
            message,
            OAEP(
                mgf=MGF1(algorithm=SHA256()),
                algorithm=SHA256(),
        ...
        print(f"Attacker intercepted and decrypted the message: {plaintext}")

        # Modify the message (e.g., append some data)
        modified_plaintext = plaintext + b" - modified by attacker"

        # Re-encrypt the message using the server's public key
